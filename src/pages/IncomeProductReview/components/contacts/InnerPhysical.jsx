...
const InnerPhysical = () => {
  ...
  const checkLength = () => {
    let result = calcParseDeferentError({
      input: values.length,
      valid_value: validateData.dimension_inner_length,
      parse: 0.1,
    });
    if (result.success) {
  ...
  const checkWidth = () => {
    let result = calcParseDeferentError({
      input: values.width,
      valid_value: validateData.dimension_inner_width,
      parse: 0.1,
    });
    if (result.success) {
  ...
  const checkHeight = () => {
    let result = calcParseDeferentError({
      input: values.height,
      valid_value: validateData.dimension_inner_height,
      parse: 0.1,
    });
    if (result.success) {
  ...
  const checkWeight = () => {
    let result = calcParseDeferentError({
      input: values.weight,
      valid_value: validateData.dimension_inner_weight,
      parse: 0.1,
    });
    if (result.success) {
